...
import { readableStreamToString, WorkdayStaffingError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new WorkdayStaffingError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new WorkdayStaffingError(e, e.response?.data)
                }
                if (innerError instanceof WorkdayStaffingError) {
                    // Got "WorkdayStaffingError" from the above try block
                    throw innerError;
                }
